...
class Position:
    def __init__(self, idx, ln, col, fn, ftxt):
        ...
        self.ftxt = ftxt # text of the current line
    
    # To move the cursor to next character
...
KEYWORDS = ['VAR', 'AND', 'OR', 'NOT', 'IF', 'THEN', 'ELIF', 'ELSE', 'FOR', 'TO', 'STEP', 'WHILE', 'FUN', 'END']
...
class Parser:
    ...
    def call(self):
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected ')', 'VAR', INT, FLOAT, IDENTIFIER, '+', '-', '(', '[', or 'NOT'"))
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, "Expected INT, FLOAT, IDENTIFIER, '+', '-', '(', 'IF', 'FOR', 'WHILE', or 'FUN'"))
    ...
    def list_expr(self):
        ...
        if self.current_tok.type != TT_LSQUARE:
            return res.failur(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected '['"))
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, "Expected 'VAR', INT, FLOAT, IDENTIFIER, '+', '-', '(', '[', 'NOT', 'IF', 'FOR', 'WHILE', or 'FUN'"))
    ...
    def func_def(self):
        ...
        if self.current_tok.type != TT_NEWLINE:
            return res.faiure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected '->' or NEWLINE"))
...
class Value:
    ...
    def illegal_operation(self, other=None):
        if not other:
            other = self
        return RTError(self.pos_start, other.pos_end, f"Illegal operation", self.context)    
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_pop(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            return RTError(self.pos_start, self.pos_end, "First argument must be list", exec_ctx)
        if not isinstance(index, Number):
            return RTError(self.pos_start, self.pos_end, "Second argument must be number", exec_ctx)

        try:
            element = list_.elements.pop(index.value)
        except:
            return RTResult().failure(RTError(self.pos_start, self.pos_end, "Element at this index could not be removed because the index is out of bounds.", exec_ctx))
    ...
    def execute_extend(self, exec_ctx):
        ...
        if not isinstance(listB, List):
            return RTError(self.pos_start, self.pos_end, "Second argument must be list", exec_ctx)
